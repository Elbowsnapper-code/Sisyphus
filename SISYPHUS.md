...
37. Electron desktop — unsigned Windows zip via footer download, Save menu, About, and `/download`. Current test version **1.11.0**. About → Update pulls later desks from GitHub (`Elbowsnapper-code/Sisyphus`) without a new zip. Exe icon matches the studio mark. New Project writes under `%USERPROFILE%\\Sisyphus`.
