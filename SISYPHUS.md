...
37. Electron desktop — unsigned Windows zip via footer download, Save menu, About, and `/download`. Current test version **1.13.0**. About → Update pulls later desks from GitHub (`Elbowsnapper-code/Sisyphus`) without a new zip. Exe icon is the figure, stone, and slope. New Project writes under `%USERPROFILE%\\Sisyphus`.
